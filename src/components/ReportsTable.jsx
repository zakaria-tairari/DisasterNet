...
const ReportsTable = ({ reports }) => {
  ...
  <div className="flex items-center text-[10px] text-slate-600 dark:text-slate-300">
    ...
    <TimeDot time={report.createdAt} label="Created" />
    <TimeLine />

    <TimeDot time={report.dispatchedAt} label="Dispatched" />
    <TimeLine />

    <TimeDot time={report.onSiteAt} label="on Site" />
    <TimeLine />

    <TimeDot time={report.resolvedAt} label="Resolved" />
      ...
      <SidePanel
        ...
        title={
          selectedReport?.status === "pending"
            ? "Assign a team to this incident"
            : "View team progress"
        }
      >
        {selectedReport?.status === "pending" ? (
          <TeamsTable report={selectedReport} />
        ) : (
          <TeamProgressView report={selectedReport} />
        )}
      </SidePanel>
    </>
  );

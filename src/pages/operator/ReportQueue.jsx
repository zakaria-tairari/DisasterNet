...
const ReportQueue = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, "reports"), where("region", "==", user.region), orderBy("createdAt", "desc"));
  ...
  : reports.filter(report => report.type.toLowerCase().includes(filter.toLowerCase()));
